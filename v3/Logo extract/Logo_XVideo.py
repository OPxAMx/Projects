...
def fetch_trailer(tmdb_id):
    ...
    for url in endpoints:
        ...
        # On cherche un trailer officiel YouTube
        for vid in data["results"]:
            if (
                ...
                return [{
                    "name": vid.get("name", ""),
                    "key": vid.get("key"),
                    "site": vid.get("site", "YouTube"),
                    "size": vid.get("size", 1080),
                    "type": vid.get("type", "Trailer"),
                    "official": vid.get("official", True)
                }]

        # Sinon on prend n’importe quel trailer YouTube
        for vid in data["results"]:
            if vid.get("site") == "YouTube" and vid.get("key"):
                return [{
                    ...
                    "size": vid.get("size", 1080),
                    "type": vid.get("type", "Trailer"),
                    "official": vid.get("official", False)
                }]
